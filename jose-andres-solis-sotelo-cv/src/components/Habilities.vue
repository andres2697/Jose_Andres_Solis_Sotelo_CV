...
<template>
  <div class="max-h-auto w-full flex flex-col bg-[#eb7f96] text-white p-4 rounded-xl self-center sm:max-h-screen-minus-72">
    ...
    <div class="w-full ml-0 mt-2 overflow-hidden sm:ml-4">
      <div class="w-full flex flex-col">
        ...
        <div class="w-[100%] h-9 flex justify-center self-center mb-2 ml-0 mr-0 sm:ml-12 sm:mr-8">
        ...
        <div class="flex x-full overflow-hidden sm:w-[97%]">
          <div class="flex transition-transform duration-700 ease-in-out w-full -translate-x-{{ currentPosition }}">
            ...
            <div
              ...
              :key="`${index}sm`" 
              class="w-1/2 h-[280px] flex flex-col p-4 ml-2 rounded-md bg-gradient-to-b from-red-400 to-pink-500 sm:ml-1">
                <h3 class="font-medium text-xs mb-2 sm:text-sm">{{ item.title }}</h3>
                <div v-html="item.content" class="w-full h-auto flex flex-col space-y-2 text-xs sm:text-sm"></div>
            </div>
          </div>
