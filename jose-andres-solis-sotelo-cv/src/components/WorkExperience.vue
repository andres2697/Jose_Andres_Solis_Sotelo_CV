...
<template>
  <div class="w-full flex flex-col bg-[#eb7f96] text-white p-4 rounded-xl self-center md:max-h-screen-minus-72">
    ...
    <div class="w-full flex flex-col mr-4 mt-4 mb-4">
      <div class="w-full flex mb-5">
        ...
        >
          <div class="w-full flex text-center justify-center text-xs font-semibold md:text-base">{{ trabajo.nombreEmpresa }}</div>
        </div>
      </div>
      <div
        class="w-full flex"
        ...
        <div class="w-full flex flex-col text-justify rounded-md bg-[#e96262] bg-opacity-75 p-4 ml-2 mr-2" v-if="store.selectedJob === index">
          <div class="w-full flex">
            ...
            <div class="w-full flex flex-col">
              <span class="text-sm md:text-base">{{ trabajoDetalle.descripcionPuesto }}</span>
              <span class="text-xs mt-0 md:text-sm md:-mt-1">{{ trabajoDetalle.periodo }}</span>
            </div>
          </div>
          <div class="w-full flex flex-col mt-3 md:flex-row">
            <div class="w-full flex flex-col md:w-[50%]">
              <div class="w-full flex space-x-1 justify-center text-center mb-2 md:space-x-2">
                <font-awesome-icon icon="list-check" class="mt-auto mb-auto"/>
                <h3 class="text-sm md:text-base">Actividades realizadas</h3>
              </div>
